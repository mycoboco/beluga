...
#define isempty(t) (*(t)->spell == '\0')    /* true if space token denotes empty token */
...
lex_t *(mcr_define)(int cmd)
{
    ...
    if (t->id == '(') {    /* function-like */
        ...
        while (t->id == LEX_ID) {
            ...
            pl = lst_append(pl, lst_copy(t, strg));
    ...
    while (t->id != LEX_NEWLINE && t->id != LEX_EOI) {
        if (t->id == LEX_SPACE) {
            ...
            if (u->id != LEX_NEWLINE && u->id != LEX_EOI) {
                SETSPELL(t, " ");
                l = lst_append(l, lst_copy(t, strg));
            }
            t = u;
            continue;
        } else {
        ...
        for (q = p->rlist; *q; ) {
            ...
            if (t->id == LEX_ID) {
                if (pl && (r = plookup(pl, t)) != NULL)
                    l = lst_append(lst_append(lst_append(l, lex_mcr(NULL, 0, STRG)),
                                              lst_copyl(r->elist, STRG)),
                                   lex_mcr(NULL, 1, STRG));
                else {
                    l = lst_append(l, lst_copy(t, STRG));
                    pe = elookup(t);
