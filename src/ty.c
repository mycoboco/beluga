...
static void setmaxmin(ty_t *ty)
{
    ...
    if (sign) {
        ty->u.sym->u.lim.max.li = ONES(ty->size*TG_CHAR_BIT - 1);
        ty->u.sym->u.lim.min.li = -ty->u.sym->u.lim.max.li - 1;
    } else {
        ty->u.sym->u.lim.max.ul = ONES(ty->size*TG_CHAR_BIT);
        ty->u.sym->u.lim.min.ul = 0;
    }
}
...
ty_t *(ty_array_s)(ty_t *ty, long n)
{
    ...
    if (TY_ISFUNC(ty)) {
        ...
        return ty_array_s(ty_ptr(ty), n);    /* returns array of function pointers */
    }
    if (TY_ISVOID(ty))    /* array of void */
        err_issue_s(ERR_TYPE_ARRVOID);
 ...
 */
ty_t *(ty_qual_s)(int op, ty_t *ty, int dclr)
{
    assert(op & TY_CONSVOL);
    assert(ty);

    if (TY_ISARRAY(ty))
        ty = type(TY_ARRAY, ty_qual_s(op, ty->type, dclr), ty->size, ty->align, NULL);
    else if (TY_ISFUNC(ty))
        err_issue_s(ERR_TYPE_QUALFUNC, op);
    else if (ty->op & op)
        err_issue_s((dclr)? ERR_TYPE_DUPQUALDCLR: ERR_TYPE_DUPQUAL, op);
    else {
        if (TY_ISQUAL(ty)) {
            op |= ty->op;
            ty = ty->type;
 ...
 *  calls to err_issue_s() in ty_qual_s() never invoked, so no _s suffix
 */
ty_t *(ty_qualc)(int op, ty_t *ty)
{
    ...
    op &= ~bty->op;
    return (op)? ty_qual_s(op, ty, 0): ty;
}
...
ty_t *(ty_func_s)(ty_t *ty, void *proto[], int style)    /* ty_t */
{
    ...
    if (TY_ISARRAY(ty)) {
        ...
        ty = ty_atop_s(ty);    /* func returning pointer to element */
    } else if (TY_ISFUNC(ty)) {
        err_issue_s(ERR_TYPE_FUNCFUNC);
 ...
 */
ty_t *(ty_newstruct_s)(int ctx, int op, const char *tag)
{
    sym_t *p;
    int newty = (ctx == '{' || ctx == ';');
    ...
    if (*tag == '\0')
        ...
        if ((p = sym_lookup(tag, sym_type)) != NULL && (!newty || SYM_SAMESCP(p, sym_scope))) {
            if (p->type->op != op)
                err_issue_s(ERR_TYPE_DIFFTAG, p, " a tag", &p->pos);
            else if (ctx == '{' && p->f.defined)
                err_issue_s(ERR_TYPE_STRREDEF, p, " a tag", &p->pos);
            else
                return p->type;
 ...
 */
sym_field_t *(ty_newfield_s)(const char *name, ty_t *ty, ty_t *fty)
{
    sym_field_t *p, **q;
    const char *cname;
    ...
    for (p = *q; p; q=&p->link, p=*q)
        if (p->name == name) {
            err_issue_s(ERR_TYPE_STRDUPMEM, name, "", &p->pos);
            name = sym_semigenlab();    /* helps ty_chkfield() */
        } else if (cname && p->cname == cname) {
            err_issue_s(ERR_LEX_LONGID, p->name, &p->pos);
            err_issue_s(ERR_LEX_LONGIDSTD, (long)TL_INAME_STD);
        }
        ...
        case TY_FUNCTION:
            {
                ...
                if (!p1 && !p2)
                    t1 = ty_func_s(ty, NULL, 1);
                else if (p1 && !p2)
                    t1 = ty_func_s(ty, p1, t1->u.f.oldstyle);
                else if (!p1 && p2)
                    t1 = ty_func_s(ty, p2, t2->u.f.oldstyle);
                else {
                    for (; *p1 && *p2; p1++, p2++)
                    ...
                    t1 = ty_func_s(ty, alist_toarray(tlist, strg_perm), 0);
                }
            }
            return t1;
        default:
    ...
    for (p = ty->u.sym->u.s.flist; p; p = p->link)
        if (p->name) {
            ...
            if ((q = isfield(p->name, inherited)) != NULL)
                err_issuep(&p->pos, ERR_TYPE_STRAMBMEM, p->name, "", &q->pos, top);
            else {
                if ((q = iscfield(p->name, inherited)) != NULL) {
                    err_issuep(&p->pos, ERR_LEX_LONGID, q->name, &q->pos);
                    err_issuep(&p->pos, ERR_LEX_LONGIDSTD, (long)TL_INAME_STD);
                }
 ...
 *  might intrude on lex_outpos() if given as arguments to the same call;
 *  see also lex_outpos()
 */
const char *(ty_outtype)(const ty_t *ty, int exp)
{
    ...
    for (; ty; ty = ty->type) {
        ...
        switch(ty->op) {
            case TY_CONST:
            case TY_VOLATILE:
                s = sout(s, lex_name[ty->op], " ", NULL);
                assert(ty->type);
                break;
            case TY_CONSVOL:
                s = sout(s, lex_name[TY_CONST], " ", lex_name[TY_VOLATILE], " ", NULL);
                assert(ty->type);
                break;
            case TY_STRUCT:
            case TY_UNION:
            case TY_ENUM:
                {
                    ...
                    s = (GENNAME(ty->u.sym->name))?
                            sout(s, lex_name[ty->op], " defined at ",
                                    lex_outpos(&ty->u.sym->pos), NULL):
                            sout(s, lex_name[ty->op], " ", ty->u.sym->name, NULL);
                    if (ty->op == TY_ENUM)
                        ty = ty->type;
                    assert(!ty->type);
                }
        ...
        switch(ty->op) {
            ...
            case TY_ENUM:
                ...
                if (GENNAME(ty->u.sym->name)) {
                    *pa = 1;
                    return (*s)? sout(lex_name[ty->op], " ", s, NULL): lex_name[ty->op];
                }
            ...
            case TY_VOLATILE:
                if (TY_ISPTR(ty->type))
                    s = sout(" ", lex_name[ty->op], (*s)? " ": "", s, NULL);
                else
                    return sout(lex_name[ty->op], " ", ty_outdecl(ty->type, s, pa, exp), NULL);
                break;
            case TY_CONSVOL:
                if (TY_ISPTR(ty->type))
                    s = sout(lex_name[TY_CONST], " ", lex_name[TY_VOLATILE], (*s)? " ": "", s,
                             NULL);
                else
                    return sout(lex_name[TY_CONST], " ", lex_name[TY_VOLATILE], " ",
