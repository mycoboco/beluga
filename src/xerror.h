...
xx(INPUT_LONGLINESTD,      A|B|C, "(ISO C guarantees at most %u characters)"                     )
...
xx(CONST_LONGSTRSTD,       A|B|C, "(ISO C guarantees only %u characters)"                        )
...
xx(PARSE_SKIPTOK,      P        , "(%P skipped including one here)"                              )
...
xx(PARSE_ENUMINT1,     P  |W    , "`enum' may not be compatible with `int' (see %p)"             )
xx(PARSE_ENUMINT2,     P  |W    , "promoted type of `enum' given as parameter"                   )
xx(PARSE_INITCONST,  E|P        , "initializer must be constant"                                 )
xx(PARSE_INVINIT,    E|P        , "invalid initializer; `%y' found but `%y' expected"            )
...
xx(EXPR_NOSTRUCT1,   E|P        , "struct or union required but `%y' given"                      )
xx(EXPR_NOSTRUCT2,   E|P        , "struct or union pointer required but `%y' given"              )
xx(EXPR_UNKNOWNMEM,  E|P        , "unknown member name%i"                                        )
xx(EXPR_RETINCOMP,   E|P        , "function returns an incomplete type, `%y'"                    )
xx(EXPR_ARGNOTMATCH, E|P        , "type error in %o argument to %f; `%y' found but `%y' expected")
...
xx(TYPE_BIGARRSTD,         A|B|C, "(ISO C guarantees at most %u-byte object)"                    )
...
xx(STMT_ILLRETTYPE,  E|P        , "illegal return type; `%y' found but `%y' expected"            )
