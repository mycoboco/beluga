...
#define clx_tykw()    (clx_kind[clx_tc] == LEX_CHAR)
#define clx_tyid(s)   (clx_tc == LEX_ID &&    \
                       ((clx_sym && clx_sym->sclass == LEX_TYPEDEF) || (!clx_sym && clx_tyla(s))))
#define clx_istype(s) (clx_tykw() || clx_tyid(s))
...
#define clx_ispdecl()  (clx_kind[clx_tc] == LEX_STATIC || clx_tykw() ||    \
                        (clx_tyid(CLX_TYLA) && clx_peek() != ':'))                   /* pure */
...
#define clx_ispstmt()  (clx_issstmt() && !clx_tykw() &&    \
                        (!clx_tyid(CLX_TYLA) || clx_peek() == ':'))                  /* pure */
